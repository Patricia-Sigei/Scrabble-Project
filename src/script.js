const boardContainer1 = document.getElementById("board1");
...
// Predefined letters for each board
const predefinedLetters1 = ['C', 'A', 'T','Y', 'I','O'];  // Board 1 (CAT)
...
// Create letter rack for a board
function wordRack(container, lettersArray, prefix) {
    ...
    for (let i = 0; i < lettersArray.length; i++) {
        ...
        const randomLetter = lettersArray[i];
...
// Animate the word being placed on the board horizontally
function animateWordToBoardHorizontally(prefix, lettersArray, startRow, startCol) {
    const rackTiles = [];
    
    lettersArray.forEach((_, index) => {
        rackTiles.push(document.getElementById(`${prefix}-${index}`));
    });
    ...
    function moveToBoard() {
        if (step < 3) {
            ...
            setTimeout(() => {
                ...
                step++;
                moveToBoard();  // Move to next letter
...
// Generate the board
genBoard(boardContainer1);  // Board for CAT


// Generate letter racks
const rackContainer1 = document.createElement("div");

boardContainer1.appendChild(rackContainer1);


wordRack(rackContainer1, predefinedLetters1, "cat");  // CAT rack


// Animate both words on their respective boards
animateWordToBoardHorizontally("cat", predefinedLetters1, 8, 8);  // Animate CAT horizontally
